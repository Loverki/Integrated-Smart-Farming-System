-- Enhanced Smart Farming System Database Schema
-- This includes advanced DBMS concepts: DDL, DML, TCL, Views, Joins, Stored Procedures
...
-- 5. SOIL ANALYSIS TABLE (New)
CREATE TABLE SOIL_ANALYSIS (
    analysis_id NUMBER PRIMARY KEY,
    farm_id NUMBER REFERENCES FARM(farm_id) ON DELETE CASCADE,
    analysis_date DATE NOT NULL,
    nitrogen_level NUMBER(8,2),
    phosphorus_level NUMBER(8,2),
    potassium_level NUMBER(8,2),
    organic_matter NUMBER(8,2),
    ph_level NUMBER(3,1),
...
-- 10. EQUIPMENT MAINTENANCE TABLE (New)
CREATE TABLE EQUIPMENT_MAINTENANCE (
    ...
    next_maintenance_date DATE,
    status VARCHAR2(20) DEFAULT 'COMPLETED' CHECK (status IN ('SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'))
);
...
-- 12. PESTICIDE TABLE (New)
CREATE TABLE PESTICIDE (
    pesticide_id NUMBER PRIMARY KEY,
    farm_id NUMBER REFERENCES FARM(farm_id) ON DELETE CASCADE,
    pesticide_name VARCHAR2(50) NOT NULL,
    pesticide_type VARCHAR2(30) CHECK (pesticide_type IN ('HERBICIDE', 'INSECTICIDE', 'FUNGICIDE', 'BIOLOGICAL')),
    quantity_used NUMBER(8,2),
    unit VARCHAR2(20) DEFAULT 'LITERS',
    applied_date DATE NOT NULL,
    cost_per_unit NUMBER(8,2),
    total_cost NUMBER(10,2),
    applied_by VARCHAR2(100),
...
-- 13. IRRIGATION TABLE (New)
CREATE TABLE IRRIGATION (
    irrigation_id NUMBER PRIMARY KEY,
    farm_id NUMBER REFERENCES FARM(farm_id) ON DELETE CASCADE,
    irrigation_date DATE NOT NULL,
    irrigation_type VARCHAR2(30) CHECK (irrigation_type IN ('DRIP', 'SPRINKLER', 'FLOOD', 'MANUAL')),
    water_used NUMBER(8,2),
    unit VARCHAR2(20) DEFAULT 'LITERS',
    duration_hours NUMBER(5,2),
    cost_per_liter NUMBER(8,2),
    total_cost NUMBER(10,2),
    performed_by VARCHAR2(100),
    crop_id NUMBER REFERENCES CROP(crop_id),
    weather_condition VARCHAR2(50),
...
CREATE TABLE CROP_DISEASES (
    ...
    affected_area NUMBER(8,2),
    treatment_applied VARCHAR2(200),
    treatment_cost NUMBER(10,2),
    status VARCHAR2(20) DEFAULT 'DETECTED' CHECK (status IN ('DETECTED', 'TREATING', 'RESOLVED', 'SPREADING')),
    notes CLOB
);

-- 15. SALES TABLE (Enhanced)
CREATE TABLE SALES (
...
CREATE INDEX idx_soil_analysis_farm_date ON SOIL_ANALYSIS(farm_id, analysis_date);
...
SELECT 
    ...
    fm.area,
    fm.soil_type,
    COUNT(c.crop_id) as crops_count,
    ...
    SUM(pes.total_cost) as pesticide_cost,
    SUM(irr.total_cost) as irrigation_cost,
...
LEFT JOIN PESTICIDE pes ON fm.farm_id = pes.farm_id
LEFT JOIN IRRIGATION irr ON fm.farm_id = irr.farm_id
...
SELECT 
    ...
    SUM(s.total_amount) as total_revenue,
    AVG(DATEDIFF(c.actual_harvest_date, c.sowing_date)) as avg_growth_days
FROM CROP c
...
SELECT 
    ...
    SUM(irr.total_cost) as irrigation_cost,
    SUM(lw.total_cost) as labour_cost,
    SUM(fer.total_cost + pes.total_cost + irr.total_cost + lw.total_cost) as total_operating_cost,
    SUM(s.total_amount) as total_revenue,
    ROUND((SUM(s.total_amount) - SUM(fer.total_cost + pes.total_cost + irr.total_cost + lw.total_cost)), 2) as net_profit
FROM FARM fm
JOIN FARMER f ON fm.farmer_id = f.farmer_id
LEFT JOIN FERTILIZER fer ON fm.farm_id = fer.farm_id
LEFT JOIN PESTICIDE pes ON fm.farm_id = pes.farm_id
...
LEFT JOIN LABOURWORK lw ON fm.farm_id = lw.farm_id
LEFT JOIN SALES s ON fm.farm_id = s.farm_id
GROUP BY fm.farm_id, fm.farm_name, f.name;

-- Insert Sample Data

-- Insert Admin
INSERT INTO ADMIN (admin_id, username, email, password, full_name, role)
VALUES (ADMIN_SEQ.NEXTVAL, 'admin', 'admin@smartfarming.com', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'System Administrator', 'SUPER_ADMIN');

-- Insert Farmers
INSERT INTO FARMER (farmer_id, name, phone, email, address, password, status)
VALUES (FARMER_SEQ.NEXTVAL, 'John Doe', '1234567890', 'john@example.com', 'Village 123, District A', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'ACTIVE');

INSERT INTO FARMER (farmer_id, name, phone, email, address, password, status)
VALUES (FARMER_SEQ.NEXTVAL, 'Jane Smith', '9876543210', 'jane@example.com', 'Village 456, District B', '$2a$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'ACTIVE');

-- Insert Farms
INSERT INTO FARM (farm_id, farmer_id, farm_name, location, area, soil_type, soil_ph, irrigation_type, farm_type)
VALUES (FARM_SEQ.NEXTVAL, 1, 'North Field', 'North Village', 10, 'Loamy', 6.5, 'DRIP', 'CONVENTIONAL');

INSERT INTO FARM (farm_id, farmer_id, farm_name, location, area, soil_type, soil_ph, irrigation_type, farm_type)
VALUES (FARM_SEQ.NEXTVAL, 2, 'South Garden', 'South Village', 8, 'Sandy', 7.0, 'SPRINKLER', 'ORGANIC');

-- Insert Crops
INSERT INTO CROP (crop_id, farm_id, crop_name, variety, sowing_date, expected_harvest_date, expected_yield, crop_status, seed_quantity, planting_density)
VALUES (CROP_SEQ.NEXTVAL, 1, 'Wheat', 'Golden Wheat', DATE '2024-10-01', DATE '2025-03-01', 500, 'GROWING', 50, 100);

INSERT INTO CROP (crop_id, farm_id, crop_name, variety, sowing_date, expected_harvest_date, expected_yield, crop_status, seed_quantity, planting_density)
VALUES (CROP_SEQ.NEXTVAL, 2, 'Rice', 'Basmati', DATE '2024-11-01', DATE '2025-04-01', 400, 'PLANTED', 30, 80);

-- Insert Labour
INSERT INTO LABOUR (labour_id, name, phone, email, skill, hourly_rate, address, status)
VALUES (LABOUR_SEQ.NEXTVAL, 'Mike Johnson', '5551234567', 'mike@example.com', 'Field Worker', 15.00, 'Worker Village', 'AVAILABLE');

-- Insert Weather Data
INSERT INTO WEATHER_DATA (weather_id, farm_id, recorded_date, temperature, humidity, rainfall, wind_speed, pressure, weather_condition, recorded_by)
VALUES (WEATHER_SEQ.NEXTVAL, 1, SYSDATE, 25.5, 65.0, 0.0, 12.0, 1013.25, 'SUNNY', 'AUTOMATIC_SENSOR');

-- Insert Soil Analysis
INSERT INTO SOIL_ANALYSIS (analysis_id, farm_id, analysis_date, nitrogen_level, phosphorus_level, potassium_level, organic_matter, ph_level, moisture_content, analyzed_by, recommendations)
VALUES (SOIL_SEQ.NEXTVAL, 1, SYSDATE, 120.5, 45.2, 180.3, 2.8, 6.5, 15.2, 'Soil Lab', 'Add nitrogen fertilizer for better growth');
...
BEGIN
    ...
    SELECT NVL(SUM(total_amount), 0) INTO p_revenue
    FROM SALES
    WHERE farm_id = p_farm_id;
    
    -- Calculate total costs
    SELECT NVL(SUM(total_cost), 0) INTO p_cost
    FROM (
        SELECT total_cost FROM FERTILIZER WHERE farm_id = p_farm_id
        UNION ALL
        SELECT total_cost FROM PESTICIDE WHERE farm_id = p_farm_id
        UNION ALL
        SELECT total_cost FROM IRRIGATION WHERE farm_id = p_farm_id
        UNION ALL
...
-- Grant permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON FARMER TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON FARM TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON CROP TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON SALES TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON LABOUR TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON LABOURWORK TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON FERTILIZER TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON ADMIN TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON WEATHER_DATA TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON SOIL_ANALYSIS TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON EQUIPMENT TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON EQUIPMENT_MAINTENANCE TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON PESTICIDE TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON IRRIGATION TO PUBLIC;
GRANT SELECT, INSERT, UPDATE, DELETE ON CROP_DISEASES TO PUBLIC;

COMMIT;

-- Display success message
SELECT 'Enhanced Smart Farming System Database Created Successfully!' as STATUS FROM DUAL;
