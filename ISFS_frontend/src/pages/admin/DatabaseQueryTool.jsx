...
export default function DatabaseQueryTool() {
  ...
  const prebuiltQueries = [
    { name: "All Farmers", query: "SELECT * FROM FARMER ORDER BY REG_DATE DESC FETCH FIRST 50 ROWS ONLY" },
    { name: "All Farms", query: "SELECT * FROM FARM ORDER BY FARM_ID DESC FETCH FIRST 50 ROWS ONLY" },
    { name: "Recent Sales", query: "SELECT s.*, c.CROP_NAME, f.FARM_NAME FROM SALES s JOIN CROP c ON s.CROP_ID = c.CROP_ID JOIN FARM f ON s.FARM_ID = f.FARM_ID ORDER BY s.SALE_DATE DESC FETCH FIRST 20 ROWS ONLY" },
    { name: "Crop Performance", query: "SELECT CROP_NAME, COUNT(*) as total_count, AVG(EXPECTED_YIELD) as avg_expected, AVG(ACTUAL_YIELD) as avg_actual FROM CROP GROUP BY CROP_NAME ORDER BY total_count DESC" },
    { name: "Revenue by Farm", query: "SELECT f.FARM_NAME, f.LOCATION, SUM(s.TOTAL_AMOUNT) as total_revenue FROM FARM f LEFT JOIN SALES s ON f.FARM_ID = s.FARM_ID GROUP BY f.FARM_NAME, f.LOCATION ORDER BY total_revenue DESC NULLS LAST" },
    { name: "Active Farmers with Farms", query: "SELECT fa.NAME, fa.PHONE, COUNT(f.FARM_ID) as farm_count FROM FARMER fa LEFT JOIN FARM f ON fa.FARMER_ID = f.FARMER_ID WHERE fa.STATUS = 'ACTIVE' GROUP BY fa.NAME, fa.PHONE ORDER BY farm_count DESC" },
    { name: "Labour Work Summary", query: "SELECT l.LABOUR_NAME, COUNT(lw.WORK_ID) as work_count, SUM(lw.HOURS_WORKED) as total_hours FROM LABOUR l LEFT JOIN LABOUR_WORK lw ON l.LABOUR_ID = lw.LABOUR_ID GROUP BY l.LABOUR_NAME ORDER BY total_hours DESC NULLS LAST FETCH FIRST 20 ROWS ONLY" },
    { name: "Equipment Status", query: "SELECT EQUIPMENT_NAME, EQUIPMENT_TYPE, STATUS, PURCHASE_DATE, MAINTENANCE_SCHEDULE FROM EQUIPMENT ORDER BY PURCHASE_DATE DESC FETCH FIRST 30 ROWS ONLY" }
  ];
